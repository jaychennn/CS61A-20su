...
def pingpong(n):
    ...
    def pingpong_helper(value, index, direction):
        ...
        if index % 8 == 0 or num_eights(index) != 0:
            if direction == True: 
                return pingpong_helper(value - 1, index + 1, not direction)
                # Carefully think about the '-' or '+'
            else:
                return pingpong_helper(value + 1, index + 1, not direction)
        else:
            if direction == True: 
...
"""A better and more elegant way of flipping the direction"""
    # def helper(index, state, direction):
    #     if index == n:
    #         return state
    #     if num_eights(index) != 0 or index % 8 == 0:
    #         return helper(index + 1, state - direction, direction*-1)
    #     else:
