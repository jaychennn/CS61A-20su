...
def is_swap(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    flag = False
    if abs((player_score % 100)%10 - (opponent_score %100) % 10) == (opponent_score%100)//10:
        flag = True
        # Consider num bigger than 100
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            cur_score0 = take_turn(num_rolls0, score1, dice)
            if feral_hogs == True and abs(num_rolls0 - prev_score0) == 2:
                score0 += (cur_score0 + 3)
            else: score0 += cur_score0
            prev_score0 = cur_score0
            if is_swap(score0, score1): score0, score1 = score1, score0

        if who == 1:
            num_rolls1 = strategy1(score1, score0)
            cur_score1 = take_turn(num_rolls1, score0, dice)
            if feral_hogs == True and abs(num_rolls1 - prev_score1) == 2:
                score1 += (cur_score1 +3)
            else: score1 += cur_score1
    ...
    """
    Treat the turn before the first turn as scoring 0 points. 
    Hint: Do not take into account any previous feral hog bonuses or swine swap (next rule)
    when calculating the number of points scored the previous turn.
